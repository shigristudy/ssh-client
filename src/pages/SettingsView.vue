      ...
      <n-card title="GitHub Settings">
        <n-space vertical>
          ...
          <n-form :model="form" label-placement="left" label-width="160">
            ...
            <n-form-item label="Current Gist">
              <n-space vertical>
                <n-tag v-if="settingsStore.gistId" type="info" :bordered="false">
                  {{ settingsStore.gistId }}
                </n-tag>
                <n-text v-else depth="3">
                  No Gist connected. One will be created automatically when syncing.
                </n-text>
              </n-space>
            </n-form-item>
...
import { 
  ...
  NButton, NSpace, NTag, NAlert, NInputGroup, NText, useMessage 
} from 'naive-ui'
...
const lastSync = ref<number | null>(settingsStore.lastSync || null)
...
// Watch theme changes
watch(() => generalSettings.theme, async () => {
  await settingsStore.saveSettings()
}, { deep: true })
